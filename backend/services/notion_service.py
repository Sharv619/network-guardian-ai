...
def fetch_history() -> list[ThreatEntry]:
    ...
    history = []
    try:
        response = notion.databases.query(database_id=settings.NOTION_DATABASE_ID, page_size=20)  # type: ignore[attr-defined]
        for page in response.get("results", []):
            ...
            domain = (
                domain_prop[0].get("text", {}).get("content", "Unknown")
                if domain_prop
                else "Unknown"
            )
