...
def get_vector_memory():
    from .vector_store import VectorMemory, vector_memory
...
class KnowledgeBase:
            ...
            for domain_obj in domains:
                try:
                    entry = KnowledgeEntry(
                        ...
                        timestamp=getattr(
                            domain_obj.timestamp, "isoformat", lambda: datetime.now(UTC).isoformat()
                        )()
                        if getattr(domain_obj, "timestamp", None)
                        else datetime.now(UTC).isoformat(),
                    )

                    cache_key = self._generate_cache_key(
                        getattr(domain_obj, "domain", "unknown") or "unknown"
                    )
                    ...
                    logger.error(
                        f"Error loading domain {getattr(domain_obj, 'domain', 'unknown')} into knowledge cache: {e}"
                    )
    ...
    def query_knowledge(
        self, domain: str, context: dict | None = None, k: int = 5
        ...
        search_text = (
            f"{domain} {json.dumps(context_safe, sort_keys=True)}" if context_safe else domain
        )
    ...
    def learn_from_feedback(
        ...
        corrected_risk_score: str | None = None,
    ):
    ...
    def get_statistics(self) -> dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "recent_learning_rate": len(
                    [k for k in self.knowledge_cache.values() if k.source == "human_feedback"]
                ),
                "knowledge_coverage": {
                    "high_confidence": len(
                        [k for k in self.knowledge_cache.values() if k.confidence > 0.8]
                    ),
                    "medium_confidence": len(
                        [k for k in self.knowledge_cache.values() if 0.5 <= k.confidence <= 0.8]
                    ),
                    "low_confidence": len(
                        [k for k in self.knowledge_cache.values() if k.confidence < 0.5]
                    ),
                },
            }
        except Exception as e:
...
def get_knowledge_base(db_session: Session | None = None) -> KnowledgeBase:
...
def analyze_with_knowledge_base(
    domain: str, context: dict | None = None, fallback_to_api: bool = True
):
    """Enhanced analysis using knowledge base with fallback options"""

    # Create a synchronous session for this analysis
    # Handle both asyncpg and aiosqlite URLs
