...
def test_analyze_endpoint_fallback():
    ...
    with patch("backend.api.router.analyze_domain", side_effect=Exception("API Down")):
        try:
            client.post("/analyze", json={"domain": "error-test.com"})
        except Exception:
...
def test_chat_graceful_degradation():
    ...
    with patch("backend.api.chat.chat_with_ai", side_effect=Exception("429 Too Many Requests")):
        response = client.post("/chat", json={"message": "hello"})
        assert response.status_code == 200
        assert "chat unavailable" in response.json()["text"].lower()
