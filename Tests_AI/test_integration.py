...
class TestHealthEndpoints:
    ...
    def test_system_endpoint(self, sync_client):
        ...
        response = sync_client.get("/api/stats/system")
        assert response.status_code == 200
        data = response.json()
...
class TestChatEndpoints:
    ...
    def test_chat_basic(self, sync_client):
        ...
        with patch("backend.api.chat.chat_with_ai", return_value=mock_response):
            response = sync_client.post(
                "/chat",
    ...
    def test_chat_graceful_degradation(self, sync_client):
        ...
        with patch("backend.api.chat.chat_with_ai", side_effect=Exception("API Error")):
            response = sync_client.post(
                "/chat",
                json={"message": "Hello"}
            )
