...
from backend.services.gemini_analyzer import analyze_domain as gemini_analyze
    ...
    try:
        ...
        if domain:
            ...
            for domain_obj in domains:
                try:
                    result = await log_threat_to_sheet(domain_obj)
                    results.append({"domain": domain_obj.domain, "success": True, "result": result})
                except Exception as e:
                    results.append({"domain": domain_obj.domain, "success": False, "error": str(e)})
...
async def chat_with_system(message: str, context: Optional[str] = None) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"response": response, "timestamp": datetime.utcnow().isoformat() + "Z"}
    except Exception as e:
        return {
